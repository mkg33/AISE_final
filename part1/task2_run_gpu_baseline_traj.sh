...
DEVICE="${DEVICE:-cuda}"
OUT_DIR="${OUT_DIR:-task2_output_gpu_baseline_traj}"
...
TRAJ_INTERVAL="${TRAJ_INTERVAL:-50}"
TRAJ_INCLUDE_LBFGS="${TRAJ_INCLUDE_LBFGS:-}"

python3 task2_train.py \
  --device "${DEVICE}" \
  --out-dir "${OUT_DIR}" \
  --k-values ${K_VALUES} \
  ...
  --save-trajectory \
  --trajectory-interval "${TRAJ_INTERVAL}" \
  ${TRAJ_INCLUDE_LBFGS}
