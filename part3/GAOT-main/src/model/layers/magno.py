...
class MAGNOConfig:
    ...
    def __post_init__(self):
        ...
        if self.dynamic_radius:
            if self.dynamic_radius_k <= 0:
                raise ValueError("dynamic_radius_k must be > 0 when dynamic_radius is enabled")
            if self.dynamic_radius_alpha <= 0:
                raise ValueError("dynamic_radius_alpha must be > 0 when dynamic_radius is enabled")
...
class MAGNOEncoder(nn.Module):
    ...
    def _compute_neighbors(self, x_coord: torch.Tensor, latent_coord: torch.Tensor, 
        ...
        if mode == 'fx':
            ...
            for b in range(batch_size):
                ...
                latent_b = latent_coord[b] if latent_coord.dim() == 3 else latent_coord
                for scale in self.scales:
                    scaled_radius = self.config.radius * scale
                    neighbors = self.nb_search(
    ...
    def forward(self, 
        ...
        latent_tokens_coord : torch.Tensor
            ...
            - fx mode: [num_latent_nodes, coord_dim]
            - vx mode: [num_latent_nodes, coord_dim] or [batch_size, num_latent_nodes, coord_dim]
        ...
        if coord_mode == 'fx':
            if latent_tokens_coord.dim() != 2 or latent_tokens_coord.shape[1] != self.coord_dim:
                raise ValueError(
            ...
            if latent_tokens_coord.dim() == 2:
            ...
            elif latent_tokens_coord.dim() == 3:
                ...
                raise ValueError(
                    f"latent_tokens_coord must be 2D or 3D, got {latent_tokens_coord.shape}"
                )
