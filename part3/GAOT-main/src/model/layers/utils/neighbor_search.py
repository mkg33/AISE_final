...
def compute_knn_radius(points: torch.Tensor,
                       k: int,
                       alpha: float,
                       fallback_radius: Optional[float] = None) -> torch.Tensor:
    num_points = points.size(0)
    if num_points == 0:
        return torch.empty(0, device=points.device, dtype=points.dtype)
    if num_points == 1:
        base = fallback_radius if fallback_radius is not None else 0.0
        return torch.full((1,), base, device=points.device, dtype=points.dtype)

    k_eff = max(1, min(k, num_points - 1))
    with torch.no_grad():
        dists = torch.cdist(points, points)
        dists.fill_diagonal_(float("inf"))
        kth = torch.topk(dists, k_eff, largest=False).values[:, -1]
    return kth * alpha


def compute_query_knn_radius(queries: torch.Tensor,
                             data: torch.Tensor,
                             k: int,
                             alpha: float,
                             fallback_radius: Optional[float] = None) -> torch.Tensor:
    ...
    if num_data == 0:
        base = fallback_radius if fallback_radius is not None else 0.0
        return torch.full((num_queries,), base, device=queries.device, dtype=queries.dtype)

    k_eff = max(1, min(k, num_data))
    with torch.no_grad():
